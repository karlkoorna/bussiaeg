...
class ViewStop extends Component {
	...
	render() {
		const { t } = this.props;
		const { stop, trips, isFavorite, isLoading } = this.state;
				...
				<main id="stop" className="view">
					...
					<ol id="stop-trips">
						{isLoading ? <Loader /> : trips.length ? trips.map((trip) => {
							const [ primaryColor, secondaryColor ] = iconColors[trip.type];
							
							return (
								<li key={`${trip.name}${trip.type}${trip.time}`}>
									<Link className="stop-trips-trip" to={`/vehicle?id=${trip.route_id}`}>
