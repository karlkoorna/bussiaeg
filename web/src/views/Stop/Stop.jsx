...
class ViewStop extends Component {
	...
	fetchTimes = async () => {
		...
		try {
			const times = await (await fetch(`${process.env['REACT_APP_API']}/times?stop_id=${this.state.stop.id}`)).json();
	...
	async componentDidMount() {
		...
		try {
			const stop = await (await fetch(`${process.env['REACT_APP_API']}/stops?id=${(new URLSearchParams(window.location.search)).get('id')}`)).json();
	...
	render() {
		const { t } = this.props;
		const { stop, times, isFavorite, isLoading } = this.state;
		...
		return (
			<>
				...
				<main id="stop" className="view">
					<div id="stop-info-bar" />
					{stop.id ? (
						<div id="stop-info">
							<Icon id="stop-info-icon" shape="stop" type={stop.type} />
							<div>
								<div id="stop-info-name">{stop.name || ''}</div>
								<div id="stop-info-description">{stop.description || ''}</div>
							</div>
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" id="stop-info-favorite" className={isFavorite ? 'is-active' : ''} onClick={this.toggleFavorite}>
					...
					<ol id="stop-times">
						{isLoading ? <Loader /> : times.length ? times.map((time) => {
							const [ primaryColor, secondaryColor ] = iconColors[time.type];
							...
							return (
								<li key={`${time.name}${time.type}${time.time}`}>
									<Link className="stop-times-time" to={`/route?id=${time.route_id}`}>
										...
										<div className={'stop-times-time-countdown' + (time.countdown < 60 ? ' is-urgent' : '')}>
											{formatCountdown(time.countdown)}
											{time.live ? (
										...
										<div className="stop-times-time-time">{formatTime(time.time)}</div>
									</Link>
								</li>
							);
						}) : (
