...
class ViewStop extends Component {
	...
	fetchTimes = async () => {
		...
		try {
			const times = await (await fetch(`${process.env['REACT_APP_API']}/stops/${this.state.stop.id}/departures`)).json();
		...
		return (
			<>
				...
				<main id="stop" className="view">
					<div id="stop-info" style={stop.id ? { backgroundColor: iconColors[stop.type][0] } : {}}>
						{stop.id ? (
							<>
					...
					<ol id="stop-times">
						{isLoading ? <Loader /> : times.length ? times.map((time) => {
							...
							return (
								<li key={time.tripId}>
									<Link className="stop-times-time" to={`/route?id=${time.routeId}&trip_id=${time.tripId}&stop_id=${stop.id}`}>
										...
										<div className={'stop-times-time-countdown' + (time.countdown < 60 ? ' is-urgent' : '')}>
											{formatCountdown(time.countdown, time.live)}
											{time.live ? (
