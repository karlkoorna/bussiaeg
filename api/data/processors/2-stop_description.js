...
module.exports = async () => {
	const stops = await db.query(`
		SELECT
			stop_id AS id,
			GROUP_CONCAT(type, ':', destination) AS destinations
		FROM stop_times
	...
	for (const stop of stops) {
		...
		query += `UPDATE stops SET description = '${_(destinations.map((destination) => destination.split(':')[1])).countBy().entries().maxBy('[1]')[0]}' WHERE id = '${stop.id}';`;
	}
